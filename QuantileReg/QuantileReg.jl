...
function θcond(s::Sampler, θ::Real, β::AbstractVector{<:Real})
    z  = s.y-s.X*β
    n = length(z)
    a = gamma(1+1/θ)^θ * (sum((.-z[z.<0]).^θ)/s.α^θ + sum(z[z.>0].^θ)/(1-s.α)^θ)
...
function sampleσ(s::Sampler, θ::Real, β::AbstractVector{<:Real})
    z = s.y - s.X*β
    b = gamma(1+1/θ)^θ * (sum((.-z[z.<0]).^θ) / s.α^θ + sum(z[z.>0].^θ) / (1-s.α)^θ)
    return (rand(InverseGamma(length(z)/θ, b), 1)[1])^(1/θ)
end

function logβCond(β::AbstractVector{<:Real}, s::Sampler, θ::Real, σ::Real)
    z = s.y - s.X*β
    return - gamma(1+1/θ)^θ/σ * (sum((.-z[z.< 0]).^θ) / s.α^θ + sum(z[z.>0].^θ) / (1-s.α)^θ)
end
