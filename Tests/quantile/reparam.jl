...
β, θ, σ = mcmc(par, 0.9, 100., 0.75, inv(X'*X)*X'*log.(y), 1., 2.);
...
β, θ, σ = mcmc(par, 0.9, 100., .1, 0.03, inv(X'*X)*X'*log.(y), 2., 1., false);

"""
# MH, need to look into the mala bit
α =  0.1, ε = .1, 0.03, thin = 5, MALA = false
α =  0.9, ε = .1, 0.03, thin = 5, MALA = false
...
α = 0.9, ε = 0.75, thin=5
β₁ = [-27.839, 16.271, 0.458, -0.204, 16.535, -0.001, 0.037, -0.782, -0.145]
θ₁ = 0.154
σ₁ = 0.19
"""
par = MCMCparams(y, X, 100000, 5, 1);
β, θ, σ = mcmc(par, 0.9, 100., 0.75, inv(X'*X)*X'*log.(y), 0.167, 0.206); # scale mix rep
...
plot(θ)
plot!(cumsum(θ)./(1:length(θ)), lw = 2.5)
median(θ)
